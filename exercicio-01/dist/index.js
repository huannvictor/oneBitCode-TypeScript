...
function firstMenuOption() {
    ...
    const crewLimit = Number(prompt(`Insira a quantidade limite de tripulantes: `));
...
function secondMenuOption() {
    ...
    const name = prompt(`Insira o nome da nave que ${member} deverá ser designado:`);
    const spaceship = findSpaceship(name);
    if (spaceship) {
        const confirmation = confirm(`Confirma a inclusão de ${member} na tripulação de ${spaceship.name}`);
        if (confirmation) {
